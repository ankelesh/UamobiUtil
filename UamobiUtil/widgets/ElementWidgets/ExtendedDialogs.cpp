...
LoginPassDialog::LoginPassDialog(QString& logbuff, QString& passbuff, QWidget* parent)
	...
	loginBuff(logbuff), passBuff(passbuff)
{
	...
	loginField->setText(logbuff);
	passField->setText(passbuff);
#ifdef Q_OS_ANDROID
...
int LoginPassDialog::ascLoginPass(QString& login, QString& password)
{
	int temp;
	LoginPassDialog* lpd = new LoginPassDialog(login,password);
	temp = lpd->exec();
	lpd->deleteLater();
	return temp;
}
...
void LoginPassDialog::okPressed()
{
	if (!(loginField->text().isEmpty()) && (loginField->text().count() < 20))
	{
		...
		{
			loginBuff = loginField->text();
			passBuff = passField->text();
			emit done(confirmed);
		}
	}
}
...
#else
	QObject::connect(infoButton, SIGNAL(toggled(bool)), this, SLOT(infoToggled()));
	QObject::connect(okButton, SIGNAL(clicked()), this, SLOT(okPressed()));
